...
>
  <div class={`
    ...
    bg-surface border focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20
  ...
  `}>
    <button 
      ...
      use:tooltip={"Switch Search Engine"}
    >
      <img 
        src={activeEngine.icon} 
        alt={activeEngine.name}
        class={`w-5 h-5 object-contain transition-transform duration-200 ${showEngineMenu ? 'rotate-12 scale-110' : 'group-hover/btn:scale-110'} ${appState.isDark ? 'invert' : ''}`} 
      />
    ...
    <Input 
      ...
      class="border-none shadow-none bg-transparent focus:border-none focus:ring-0 h-10 py-0 pl-3 pr-2 text-sm placeholder:text-text-dim/60"
  ...
  {#if showEngineMenu}
    <div class="absolute top-full left-0 mt-2 w-48 p-2 bg-surface border border-border rounded-xl shadow-float animate-fade flex flex-col gap-1 origin-top-left z-50">
      {#each Object.values(SEARCH_ENGINES) as engine}
        <button
          onclick={() => switchEngine(engine.id)}
          class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 w-full text-left text-text hover:bg-bg active:scale-95"
        >
          <img 
            src={engine.icon} 
            alt={engine.name}
