...
<div class="relative w-full col-span-2 md:col-span-1 md:w-full md:max-w-[640px] lg:max-w-[720px] justify-self-center order-last md:order-none z-20" {@attach clickOutside}>
  ...
  <div transition:fade={{ duration: ANIMATION_SPEED.FADE_FAST }} class="absolute top-full left-0 mt-2 w-48 p-2 bg-surface border border-border rounded-xl shadow-float flex flex-col gap-1 origin-top-left z-50">
      {#each Object.values(SEARCH_ENGINES) as engine}
        <button onclick={() => switchEngine(engine.id)} class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200 w-full text-left text-text hover:bg-bg active-press-icon">
