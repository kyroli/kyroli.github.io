  ...
  const visualGroups = $derived.by(() => {
    ...
    if (dndState.type === 'group') {
        ...
        if (dndState.hoverId && sourceGroup) {
             ...
             if (insertIdx === -1) insertIdx = groups.length; // 没找到则追加到末尾

             // 关键修改：标记 isPlaceholder 为 true
             groups.splice(insertIdx, 0, { ...sourceGroup, isPlaceholder: true });
        } else if (sourceGroup) {
            // 如果拖到了无效区域（没有目标），把占位符放回原处，防止列表闪烁
            groups.splice(srcIdx, 0, { ...sourceGroup, isPlaceholder: true });
        }
    }
