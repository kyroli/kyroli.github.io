<script lang="ts">
  ...
  const headerClass = "flex items-center gap-3 pb-3 px-1 h-10 mt-3 border-b border-border/50 box-border";
  const headerBarClass = "h-4 w-24 bg-border/50 rounded-full animate-pulse";
  ...
  const titleClass = "h-3.5 w-3/4 bg-border/50 rounded-full animate-pulse";
  const subtitleClass = "h-2.5 w-1/2 bg-border/30 rounded-full animate-pulse";
</script>
