import { DND_CONSTANTS } from '../constants';
...
class DndEngine {
  ...
  #onInput = (e: PointerEvent) => {
    ...
    if (!this.isDragging) {
      ...
      if (dx * dx + dy * dy > DND_CONSTANTS.DRAG_THRESHOLD * DND_CONSTANTS.DRAG_THRESHOLD) {
        this.#startEngine();
      }
    }
  };
  ...
  #loop = () => {
    ...
    const dist =
      Math.abs(centerX - this.#lastCollisionX) + Math.abs(centerY - this.#lastCollisionY);
  ...
  #calculateScrollDelta(dt: number): number {
    ...
    if (viewportY < DND_CONSTANTS.SCROLL_ZONE) {
      const intensity = (DND_CONSTANTS.SCROLL_ZONE - viewportY) / DND_CONSTANTS.SCROLL_ZONE;
      return -1 * DND_CONSTANTS.SCROLL_SPEED_PER_SEC * dt * (intensity * intensity);
    } else if (viewportY > viewportH - DND_CONSTANTS.SCROLL_ZONE) {
      const intensity =
        (viewportY - (viewportH - DND_CONSTANTS.SCROLL_ZONE)) / DND_CONSTANTS.SCROLL_ZONE;
      return 1 * DND_CONSTANTS.SCROLL_SPEED_PER_SEC * dt * (intensity * intensity);
    }
    return 0;
  }
  ...
  #resolveSiteCollision(x: number, y: number) {
    ...
    for (const meta of this.#groupMetas) {
      ...
      if (this.hoverGroupId === meta.id) {
        bottom += DND_CONSTANTS.VIRTUAL_EXPANSION;
      }
